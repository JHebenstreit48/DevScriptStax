...
function makeNewSectionFile(args: {
  ...
  for (const t of args.topics) {
    ...
    lines.push(`import ${varName} from '@/routes/individual/topics/${args.sectionFolder}/${fileBase}';`);
  }
...
export function syncSections(args: {
  ...
  for (const t of target.topicsToEnsure) {
    ...
    const importLine = `import ${varName} from '@/routes/individual/topics/${target.sectionFolder}/${fileBase}';`;
